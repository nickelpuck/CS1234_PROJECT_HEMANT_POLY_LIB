...
Share *shamir_split(long secret, int n, int k, long prime) {
    ...
    // Build random polynomial of degree k-1 with constant term = secret
...
int shamir_verify(Share *shares, int k, Share test, long prime) {
    // Reconstruct polynomial value at test.x using k shares
    long computed = 0;
    for (int i = 0; i < k; i++) {
        long xi = shares[i].x;
        long yi = shares[i].y;
...
long *lagrange_interpolate_zp(long *xs, long *ys, int k, long prime) {
    ...
    // Compute polynomial coefficients via Lagrange over Z_p
    for (int i = 0; i < k; i++) {
        long denom = 1;
        for (int j = 0; j < k; j++) {
            if (i == j) continue;
        ...
        // Build product polynomial (x - xs[j]) for j != i
...
long fp_roll(long prev_fp, char old_char, char new_char,
    ...
    long h = prev_fp;
    // Remove contribution of old_char (at position 0)
    ...
    // Add new_char at position window-1
...
int *rabin_karp_search(const char *text, const char *pattern, int *count) {
    *count = 0;
    ...
    // Large prime and base for fingerprinting
    ...
    for (int i = 0; i <= n - m; i++) {
        ...
        if (i < n - m) {
            text_fp = fp_roll(text_fp, text[i], text[i + m],
                              m, fp, base_pow);
        }
    }
    return results;
