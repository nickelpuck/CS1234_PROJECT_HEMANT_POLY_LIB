...
void demo_roots() {
    ...
    int count; double *roots = poly_roots_real(q, &count);
    printf("Roots: "); for (int i=0;i<count;i++) printf("%.4g  ",roots[i]); printf("\n");
...
void demo_shamir() {
    ...
    // Reconstruct from different k shares
...
void demo_reed_solomon() {
    ...
    // k=4 message symbols, n=7 codeword symbols => can fix up to 1 erasure
    int k = 4, n = 7;
    RSCode *rs = rs_create(n, k);
    double message[] = {3, 1, 4, 1};  // polynomial: x^3 + 4x^2 + x + 3
...
void demo_error_correct() {
    ...
    // Polynomial f(x) = x^2 + 2x + 3, evaluated at 6 points
