...
void poly_print(const Poly *p)                  { _print_poly(p, stdout); }
void poly_print_to_file(const Poly *p, FILE *f) { _print_poly(p, f); }
...
Poly *poly_load(const char *filename) {
    ...
    if (!f) return NULL;
    int degree;
    if (fscanf(f, "%d", &degree) != 1 || degree < 0) { fclose(f); return NULL; }
    Poly *p = poly_create(degree);
    if (!p) { fclose(f); return NULL; }
    for (int i = 0; i <= degree; i++) {
        if (fscanf(f, "%lf", &p->coeffs[i]) != 1) {
            poly_destroy(p); fclose(f); return NULL;
        }
    }
    fclose(f);
    return poly_normalize(p);
